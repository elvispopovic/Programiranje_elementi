...
void Cleanup()
{
    ...
    {
        for(j=0, ptTourArc=bestCostTour.arcs; j<prData.dim; j++, ptTourArc++)
    ...
    if(bestCostTour.passList!=nullptr)
        delete[] bestCostTour.passList; //tour passengers
...
void setBestTour()
{
    ...
    bestCostTour.passList = new pass*[prData.nPass+1];
    for(j=0, pptCar=bestCostTour.carList; j<prData.nCars+1; j++,pptCar++)
        *pptCar = nullptr;
    for(j=0, pptPass=bestCostTour.passList; j<prData.nPass+1; j++,pptPass++)
        *pptPass = nullptr;
