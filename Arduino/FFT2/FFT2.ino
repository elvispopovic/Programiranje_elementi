...
#define BITOVA 8 //za Uno 4 je maksimalno, za MKR1010 može do 8
...
int analogPin1 = A3, analogPin2 = A4; //pinovi nisu konfigurirani (npr. INPUT_PULLUP)
...
void primjer()
{
  ...
  Serial.print("Uzoraka: "); Serial.println(uzoraka);
  for(i=0; i<uzoraka; i++)
  {
    ulazTestRe[i] = analogRead(analogPin1)/1024.0;
    ulazTestIm[i]  =analogRead(analogPin2)/1024.0;
  }
  Serial.println("Ulazni podaci (realni i imaginarni):");
  for(i=0; i<uzoraka; i++)
  {
    Serial.print(ulazTestRe[i]); Serial.print(" ");
  }
  Serial.println("");
  for(i=0; i<uzoraka; i++)
  {
    Serial.print(ulazTestIm[i]); Serial.print(" ");
  }
  Serial.println("");
  pocetak = millis();
  DCT(ulazTestRe, izlazTestRe);
  kraj = millis();
  Serial.print("DCT ("); Serial.print(kraj-pocetak); Serial.println(" ms):");
  for(i=0; i<uzoraka; i++)
  {
    Serial.print(izlazTestRe[i],5); Serial.print(" ");
  }
  Serial.println("");
  pocetak = millis();
  DST(ulazTestRe, izlazTestRe);
  kraj = millis();
  Serial.print("DST ("); Serial.print(kraj-pocetak); Serial.println(" ms):");
  for(i=0; i<uzoraka; i++)
  {
    Serial.print(izlazTestRe[i],5); Serial.print(" ");
  }
  ...
  DFT(ulazTestRe, ulazTestIm, izlazTestRe, izlazTestIm);
  kraj = millis();
  Serial.print("DFT (realni i imaginarni, "); Serial.print(kraj-pocetak); Serial.println(" ms):");
  for(i=0; i<uzoraka; i++)
  {
    Serial.print(izlazTestRe[i],5); Serial.print(" ");
  }
  ...
  for(i=0; i<uzoraka; i++)
  {
    Serial.print(izlazTestIm[i],5); Serial.print(" ");
  }
  Serial.println("");
  promijeniSmjer(-1);
  pocetak = millis();
  DFT(izlazTestRe, izlazTestIm, ulazTestRe, ulazTestIm);
  normiranje(ulazTestRe, ulazTestIm, 2);
  kraj = millis();
  Serial.print("IDFT (realni i imaginarni, "); Serial.print(kraj-pocetak); Serial.println(" ms):");
  for(i=0; i<uzoraka; i++)
  {
    Serial.print(ulazTestRe[i],2); Serial.print(" ");
  }
  Serial.println("");
  for(i=0; i<uzoraka; i++)
  {
    Serial.print(ulazTestIm[i],2); Serial.print(" ");
  }
  Serial.println("");
  promijeniSmjer(0);
  Serial.println("=====================");
