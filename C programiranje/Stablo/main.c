...
cvor** kreirajStablo(const unsigned int* veze, unsigned int n)
{
    ...
    for(i=0, pptCvor = cvorovi; i<n; i++, pptCvor++)
    {
        *pptCvor = malloc(sizeof(cvor));
        if(i<9)
            sprintf((*pptCvor)->oznaka, "N_0%u",i+1);
        else
            sprintf((*pptCvor)->oznaka, "N_%u",i+1);
        (*pptCvor)->brojDjece=0;
        (*pptCvor)->djeca=NULL;
    }
...
stog* kreirajStog(unsigned int n)
{
    ...
    s->elementi = malloc(n*sizeof(cvor*));
    s->dno = (s->elementi)+n-1;
    s->pptPokazivac = s->dno;
    return s;
}

void obrisiStog(stog* s)
{
    free(s->elementi);
    free(s);
}
...
cvor** obilazenjePoDubiniPreorder(cvor** stablo, stog* s, unsigned int n, char smjer)
{
    int i;
    cvor **pptDjeca;
...
cvor** obilazenjePoDubiniPostorder(cvor** stablo, stog* s, unsigned int n, char smjer)
{
    int i;
    cvor **pptDjeca;
    cvor **rezultat, **pptRezultat;
...
int main()
{
   ...
   ima 22 elementa, to su cvorovi 1,2,3,...22, a uz nulti to je ukupno 23 cvora*/
    ...
    rezultat = obilazenjePoDubiniPreorder(stablo, s, n, 1);
    ispisiCvorove(rezultat,n, false);
    free(rezultat);
    printf("Obilazenje postorder:\n");
    rezultat = obilazenjePoDubiniPostorder(stablo, s, n, 1);
    ispisiCvorove(rezultat,n, false);
    free(rezultat);
