...
void updateBestCostTour(ant* ptAnt)
{
    ...
    {
        bestCostTour.ant = ptAnt->t.ant;
        bestCostTour.iteration = ptAnt->t.iteration;
        ...
        *pptCar2 = nullptr;
        ...
        for(j=0, pptPass1=ptAnt->t.passList, pptPass2=bestCostTour.passList; j<prData.nPass && *pptPass1!=nullptr; j++, pptPass1++, pptPass2++)
        {
            *pptPass2 = *pptPass1; //list of passengers
        }
        *pptPass2 = nullptr;
        bestCostTour.cost = ptAnt->t.cost;
        bestCostTour.costNoPass = ptAnt->t.costNoPass;
    }
}
...
void UpdatePheromoneTrails()
{
    ...
    cost = ptBestAnt->t.cost;
    if(cost<1.0)
        cost = 1.0;

    fc = feroQ/cost; 
    tau_max = 1.0/(1.0-RHO)*fc;
    tau_min = tau_max/(2.0*prData.dim);

    for(ptTourArc=ptBestAnt->t.arcs; ptTourArc->a != nullptr; ptTourArc++)
    {
        /* 
        * arcs pheromone trails
        * same pheromone trail strength for all arcs
        */

        ptTourArc->a->tau += fc;
        if(ptTourArc->a->tau > tau_max)
            ptTourArc->a->tau = tau_max;
        if(ptTourArc->a->tau < tau_min)
    ...
    for(pptPass=ptBestAnt->t.passList; *pptPass!=nullptr; pptPass++)
        (*pptPass)->tau += fc; 
